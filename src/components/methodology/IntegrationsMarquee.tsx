...
export default function IntegrationsMarquee() {
    return (
        <section className="mt-32 mb-12 relative text-center w-full">
            <h3 className="text-ignitera-500 text-xs md:text-sm font-bold tracking-[0.3em] uppercase mb-4 drop-shadow-[0_0_8px_rgba(255,77,0,0.4)]">
                Integration Sources
            </h3>
            ...
            <div className="relative w-full overflow-hidden h-24 flex items-center">
                ...
                >
                    {repeatedTools.map((item, index) => (
                        <div
                            key={index}
                            className="flex items-center justify-center space-x-3 text-zinc-600 hover:text-white hover:drop-shadow-[0_0_10px_rgba(255,255,255,0.5)] transition-all duration-300 whitespace-nowrap px-8 md:px-12 cursor-pointer"
                        >
                            <item.icon className="w-7 h-7 md:w-8 md:h-8 opacity-70" />
